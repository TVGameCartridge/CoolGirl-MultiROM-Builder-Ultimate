...
; --- LISA SEE OSA SIIN ---
  ; Enne kui panku vahetame, kopeerime loaderi uuesti RAM-i $0500 peale
  ; See tagab, et loader on värske ja kättesaadav.
  ldx #$00
.copy_fix:
  lda $C500, x ; Kuna menüü on tavaliselt $C000-$FFFF, siis ram_routines on seal
  sta $0500, x
  lda $C600, x
  sta $0600, x
  inx
  ...
  ; Nüüd laadime CHR graafika (see on okei)
  jsr load_all_chr_banks
  
  ; JÄTA SEE ÄRA või liiguta loaderi sisse:
  ; lda #0
  ; jsr select_prg_bank 
  ...
  ; HÜPPE RAM-i
  jmp $0500 ; Otse loaderi algusesse RAM-is
