...
; --- 1. Vali 10. mäng (indeks 9) ---
  lda #9
  sta <SELECTED_GAME
  lda #0
  sta <SELECTED_GAME+1

  ; --- 2. Vali ekraani alguspunkt ---
  ; Kui tahad, et 1. mäng oleks nimekirja alguses ja kursor 10-nes:
  lda #0
  sta <SCROLL_LINES_TARGET
  sta <SCROLL_LINES_TARGET+1
  
  ; --- 3. Sünkroniseeri muutujad ---
  sta <SCROLL_LINES
  sta <LAST_LINE_GAME
  sta <TMP
...
.init_modulo_end:
  ...
  ; See on kriitiline: arvutame kursorite asukoha uuesti
  jsr set_scroll_targets
...
.not_tests:
  ; 1. Seadistame SCROLL_LOCK ja algmängu
  lda #1
  sta <SCROLL_LOCK

  ; Kui soovid, et kursor algaks 10. mängu pealt (indeks 9):
  lda #9
  sta <SELECTED_GAME
  lda #0
  sta <SELECTED_GAME+1

  ; 2. JOONISTAME EKRAANI PUHTALT (kasutades meie uut loogikat)
  jsr redraw_entire_page

  ; 3. Paneme kursorid visuaalselt paika
  jsr set_scroll_targets
  lda <SPRITE_0_X_TARGET
  sta SPRITE_0_X
  lda <SPRITE_1_X_TARGET
  ...
  ; 4. Sisselülitamine on juba redraw_entire_page sees olemas, 
  ; aga igaks juhuks ootame vblanki ja rakendame paleti.
  jsr waitblank_simple
  jsr dim_base_palette_in

  ; Ära lase nuppe all hoida
